...
export default function Recover() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6">
      {/* Banner / Stat Card */}
      <div className="flex flex-col gap-4 rounded-2xl border border-amber-200/80 bg-gradient-to-r from-amber-50 to-orange-50/50 p-5 shadow-2xs sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-amber-500/20 text-amber-700">
            <Trash2 size={24} />
          </div>
          <div>
        ...
        <div className="flex items-center gap-3">
          <div className="rounded-xl border border-amber-300/60 bg-white/80 px-4 py-2 text-center shadow-2xs">
            <p className="text-lg font-bold text-amber-900">{deletedUsers.length}</p>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-amber-700">In Trash</p>
          </div>
        </div>
      </div>

      {/* Search & Action Bar */}
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      {deletedUsers.length === 0 ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-2xs">
          <p className="text-sm font-semibold text-slate-800">No matching deleted users</p>
          <p className="mt-1 text-xs text-slate-500">Check your search query or clear the filter.</p>
          <button
            onClick={() => setSearchQuery("")}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-100 text-amber-700 text-xs font-bold">
                      ...
                      {/* Role */}
                      <td className="px-6 py-4">
                        <span className="inline-flex rounded-md bg-slate-100 px-2 py-0.5 text-[11px] font-semibold capitalize text-slate-600">
                          {user.role || "user"}
                        </span>
                      </td>
